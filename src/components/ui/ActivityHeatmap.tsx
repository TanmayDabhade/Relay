...
function levelFor(count: number, sortedNonZero: number[]): 0 | 1 | 2 | 3 | 4 {
  ...
  const q = (p: number) => sortedNonZero[Math.min(sortedNonZero.length - 1, Math.floor(p * sortedNonZero.length))];
...
export function ActivityHeatmap({ data }: ActivityHeatmapProps) {
  const { weeks, monthMarkers, totalActive } = useMemo(() => {
    ...
    const monthMarkers = weeks.map((week) => {
      const firstOfMonth = week.find((cell) => cell && parseUtcDate(cell.date).getUTCDate() <= 7);
  ...
  return (
    <div className="activity-heatmap">
        ...
        <div className="activity-heatmap-body">
          <div className="activity-heatmap-day-labels">
            {DAY_LABELS.map((label, i) => (
              <span key={label} className={i % 2 === 0 ? "" : "activity-heatmap-day-hidden"}>
                {i % 2 === 1 ? label : ""}
              </span>
            ))}
          </div>
              ...
              <div className="activity-heatmap-week" key={wi}>
                {week.map((cell, di) =>
                  cell ? (
                    ...
                    <span key={di} className="activity-heatmap-cell activity-heatmap-cell-spacer" />
                  ),
                )}
              </div>
            ))}
      ...
      <div className="activity-heatmap-footer">
        <span>{totalActive} active day{totalActive === 1 ? "" : "s"} in the last year</span>
        <div className="activity-heatmap-legend">
          <span>Less</span>
          {[0, 1, 2, 3, 4].map((level) => (
            <span key={level} className={`activity-heatmap-cell activity-heatmap-level-${level}`} />
          ))}
          <span>More</span>
        </div>
      </div>
