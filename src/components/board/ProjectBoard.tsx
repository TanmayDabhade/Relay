...
export function ProjectBoard({ projectId }: ProjectBoardProps) {
  ...
  const linkableSessions = useMemo(() => {
    ...
    const linkedSessionIds = new Set(boardData.cards.map((c) => c.session_id).filter(Boolean));
    return sessions.filter((s) => s.project_id === projectId && !linkedSessionIds.has(s.id));
      ...
      <div className="project-board-columns">
        ...
        <div className="project-board-add-column">
          {isAddingColumn ? (
            ...
            <button className="board-column-add-button" onClick={() => setIsAddingColumn(true)}>
              + Add column
            </button>
          )}
        </div>
