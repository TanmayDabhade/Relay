...
export function ReportView() {
  ...
  return (
    <div className="report-view">
      ...
      {isError && (
        <p className="dashboard-view-status">Couldn't build the report. Is the backend running?</p>
      )}

      {data && (
        <>
          <div className="dashboard-stats-row">
            <StatTile value={`$${data.totals.total_cost_usd.toFixed(2)}`} label="Total spend" />
          ...
          <div className="report-export-row">
            <Button variant="secondary" onClick={handleExport} disabled={exportState.status === "saving"}>
              {exportState.status === "saving" ? "Exporting…" : "Export as Markdown"}
            </Button>
            {exportState.status === "saved" && (
              <span className="report-export-status">
                Saved to {exportState.path}
                <button className="report-export-reveal" onClick={() => revealInFinder(exportState.path)}>
                  Reveal in Finder
                </button>
              </span>
            )}
            {exportState.status === "error" && (
              <span className="report-export-status report-export-error">{exportState.message}</span>
            )}
          </div>
            ...
            <section className="dashboard-section dashboard-column">
              ...
              <div className="dashboard-card">
                {data.by_project.length === 0 ? (
                  <p className="dashboard-view-status">No activity in this window.</p>
                ) : (
            ...
            <section className="dashboard-section dashboard-column">
              ...
              <div className="dashboard-card">
                {data.by_tag.length === 0 ? (
                  <p className="dashboard-view-status">No tagged sessions in this window.</p>
                ) : (
                  <table className="report-table">
                    <thead>
                      <tr>
