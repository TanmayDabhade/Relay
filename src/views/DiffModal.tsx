 ...
 * session's edits did.
 */
export function DiffModal({ sessionId, sessionLabel, filePath, onClose }: DiffModalProps) {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["file-diff", sessionId, filePath],
    queryFn: () => getFileDiffForSessionFile(sessionId, filePath as string),
        ...
        <>
          ...
          {data.lines.length === 0 ? (
            <p className="diff-modal-status">
              No diff content captured for this change — it was recorded before Relay started
              storing before/after text.
            </p>
          ) : (
